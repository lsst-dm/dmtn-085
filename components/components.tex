...
eventually replace ci\_hsc, and a set of test scripts will be run
in a meta-package named lsst\_ci.  However, at the time of writing,
the validate\_drp scripts test only the single frame processing
step, while ci\_hsc exercises almost the entire end-to-end DRP
pipelines.  There has not been sufficient resources in implementing
...
QAWG recommend priority to unify the CI test package design and
finish the transitioning to validate\_drp. If such effort cannot
...
can replace ci\_hsc, ci\_hsc should be maintained.
